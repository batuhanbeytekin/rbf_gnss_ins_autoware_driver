            ...
            switch (status_) {
                case ParseStatus::SYNCH1_CONTROL: {
                    if (first_synch == buffer[i]) {
                        ...
                        status_ = ParseStatus::SYNCH2_CONTROL;
                    }
                    break;
                }

                case ParseStatus::SYNCH2_CONTROL: {
                    if (second_synch == buffer[i]) {
                        raw_data_[data_index_++] = buffer[i];
                        status_ = ParseStatus::SYNCH3_CONTROL;
                    } 
                    else {
                        data_index_ = 0;
                ...
                case ParseStatus::SYNCH3_CONTROL: {
                    if (third_synch == buffer[i]) {
                        raw_data_[data_index_++] = buffer[i];
                        status_ = ParseStatus::HEADER_LENGTH;
                    } 
                    else {
                        data_index_ = 0;
                        status_ = ParseStatus::SYNCH1_CONTROL;
                ...
                case ParseStatus::HEADER_ADD: {
                    raw_data_[data_index_++] = buffer[i];
                    if (data_index_ >= header_.header_length) {
                        memcpy(reinterpret_cast<uint8_t*>(&header_), raw_data_, sizeof(Header));
                        status_ = ParseStatus::DATA_ADD;
                    }
                    break;
                }
                ...
                case ParseStatus::DATA_ADD: {
                    ...
                    if (data_index_ >= header_.msg_len + header_.header_length + crc_len) {
                        uint32_t crc;
                        memcpy(reinterpret_cast<uint8_t*>(&crc), raw_data_ + header_.msg_len + header_.header_length, sizeof(uint32_t));
                        if (calculate_crc32(raw_data_, data_index_ - crc_len) == crc) {
                            if (callback_ != nullptr) {
                                callback_(raw_data_ + header_.header_length, static_cast<BinaryParser::MessageId>(header_.msg_id));
                            }
                        }
                        data_index_ = 0;
                        status_ = ParseStatus::SYNCH1_CONTROL;
